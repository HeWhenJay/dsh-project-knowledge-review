...
export async function recognizeImageUrl(config: MediaServiceConfig, imageUrl: string): Promise<string> {
  const media = await downloadPublicMedia(imageUrl, IMAGE_MAX_BYTES, config.timeoutMs)
  if (!media.contentType.startsWith('image/')) throw new Error('URL 返回的内容不是图片')
  ...
  if (typeof text !== 'string' || !text.trim()) throw new Error('OCR 服务没有返回可用文字')
  return text.trim()
}

/** 使用 OpenAI 兼容 audio/transcriptions 接口转写可直接下载的音频 URL。 */
export async function transcribeAudioUrl(config: MediaServiceConfig, audioUrl: string): Promise<string> {
  const media = await downloadPublicMedia(audioUrl, AUDIO_MAX_BYTES, config.timeoutMs)
  const form = new FormData()
  form.set('model', config.model)
  const arrayBuffer = media.bytes.buffer.slice(media.bytes.byteOffset, media.bytes.byteOffset + media.bytes.byteLength) as ArrayBuffer
  ...
  if (typeof text !== 'string' || !text.trim()) throw new Error('ASR 服务没有返回可用转写文本')
  return text.trim()
}
...
async function jsonResponse(response: Response): Promise<any> {
  ...
  let payload: any
  try { payload = JSON.parse(text) } catch { throw new Error(`模型服务返回了非 JSON 响应（HTTP ${response.status}）`) }
  if (!response.ok) {
...
async function downloadPublicMedia(rawUrl: string, maxBytes: number, timeoutMs: number): Promise<{ bytes: Buffer; contentType: string; filename: string }> {
  ...
  for (let redirect = 0; redirect < 5; redirect += 1) {
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      if (!location) throw new Error('媒体 URL 重定向缺少 Location')
      current = new URL(location, current)
      continue
    }
    if (!response.ok) throw new Error(`媒体下载失败（HTTP ${response.status}）`)
    const declared = Number(response.headers.get('content-length') ?? 0)
    if (declared > maxBytes) throw new Error(`媒体文件超过大小限制 ${Math.floor(maxBytes / 1024 / 1024)}MB`)
    const bytes = Buffer.from(await response.arrayBuffer())
    if (bytes.byteLength > maxBytes) throw new Error(`媒体文件超过大小限制 ${Math.floor(maxBytes / 1024 / 1024)}MB`)
...
async function assertPublicHttpUrl(url: URL): Promise<void> {
  if (url.protocol !== 'http:' && url.protocol !== 'https:') throw new Error('媒体 URL 只允许 http 或 https')
  if (url.username || url.password) throw new Error('媒体 URL 不允许携带用户名或密码')
  const addresses = isIP(url.hostname) ? [{ address: url.hostname }] : await lookup(url.hostname, { all: true })
  if (!addresses.length || addresses.some(({ address }) => isPrivateAddress(address))) throw new Error('媒体 URL 不能指向本机或私有网络地址')
}
