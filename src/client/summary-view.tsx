        ...
        {selected ? <>
          <div className="dsh-knowledge-reader-scroll">
            <div className="dsh-knowledge-reader-document">
              <div className="dsh-knowledge-reader-head"><div className="dsh-knowledge-reader-breadcrumb"><BookIcon /><span>知识库</span><ChevronIcon /><strong>{selected.systemCategory || '待分类'}</strong></div><h1>{selected.title}</h1><div className="dsh-knowledge-reader-meta"><span><SourceIcon />{displaySource(selected.source, '未标注来源')}</span><span><SparkleIcon />{selected.summarySource === 'model' ? 'DSH 模型摘要' : '本地自动提要'}</span><span><ClockIcon />{formatRelativeTime(selected.updatedAt || selected.createdAt)}</span></div></div>
...
/** 稳定去重键保留在存储层，界面只显示用户可理解的资料来源。 */
function displaySource(source: string | undefined, fallback: string): string { return source?.startsWith('current-project:material:') ? '当前项目导入资料' : source || fallback }
