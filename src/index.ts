...
import { addLocalDocument, resolveLocalStorePath, searchLocalKnowledge } from './local-store.js'
...
function renderProjectSearch(value: Awaited<ReturnType<typeof searchProjectKnowledge>>): string {
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.systemPrompt.section({
    ...
    text: () => {
      ...
      return settings.enabled ? buildProjectKnowledgeReviewPrompt(settings.projectName, settings.mode === 'project-rag', settings.ocrEnabled, settings.asrEnabled) : ''
    },
  })
    ...
    async execute(args) {
      ...
      try {
        if (settings.mode === 'project-rag') {
          return renderProjectSearch(await searchProjectKnowledge({ ragBaseUrl: settings.ragBaseUrl, requestTimeoutMs: settings.requestTimeoutMs }, args.question, args.topK ?? 5))
        }
        const evidences = await searchLocalKnowledge(resolveLocalStorePath(settings.localStorePath), args.question, args.topK ?? 5)
        if (!evidences.length) return refused('本地知识库没有足够证据，不能回答。请让用户粘贴学习资料文本，或在设置中启用 OCR/ASR、切换项目 RAG 模式。', '本地资料未命中')
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      const settings = current()
      if (!settings.enabled) return disabled()
      if (settings.mode !== 'project-rag') return JSON.stringify({ status: 'DISABLED', message: '当前是本地模式；视频网页 URL 需要在设置中切换为项目 RAG 模式。' })
      try {
