...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      try {
        if (projectRagEnabled) return renderProjectSearch(await searchProjectKnowledge(ragConfig, args.question, args.topK ?? 5))
        const evidences = await searchLocalKnowledge(localStorePath, args.question, args.topK ?? 5)
        if (!evidences.length) return refused('本地知识库没有足够证据，不能回答。请让用户粘贴学习资料文本，或切换到项目 RAG 模式后提供公开视频 URL。', '本地资料未命中')
