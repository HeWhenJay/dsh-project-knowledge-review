...
function renderProjectSearch(value: Awaited<ReturnType<typeof searchProjectKnowledge>>, policy: KnowledgeSettings['answerPolicy']): string {
  const data = value.data
  if (!data || data.answerStatus !== 'ANSWERED' || !data.evidences?.length) {
    if (policy === 'reference') return JSON.stringify({ answerStatus: 'REFERENCE_MISS', answerPolicy: policy, knowledgeBaseMatched: false, modelSupplementAllowed: true, requiredSections: ['知识库内容', '模型补充'], message: '知识库未命中。“知识库内容”必须明确写未命中；通用知识只能放在“模型补充”小节，不能冒充知识库结论。', evidences: [] })
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      if (!settings.enabled) return disabled()
      const intent = classifyKnowledgeIntent(args.question)
      if (intent === 'knowledge-inventory') return JSON.stringify({ answerStatus: 'ROUTE_TO_OVERVIEW', intent, message: '这是知识库状态/清单问题，请改调 project_knowledge_overview，不要执行 evidence 问答检索。', evidences: [] })
      try {
