...
interface SetupSecrets { databasePassword: string }
...
export class KnowledgeSetupService {
  ...
  constructor(
    ...
    private readonly dependencies: SetupDependencies = DEFAULT_DEPENDENCIES,
  ) {
    this.status = this.initialStatus()
  }

  defaultInstallRoot(): string { return join(this.home, 'project-knowledge-review', 'full-rag') }

  /** 返回环境与服务状态，不泄露数据库密码。 */
  async describe(): Promise<SetupStatus> {
    if (!this.task) {
      const persisted = await this.readPersistedStatus()
      if (persisted) this.status = persisted
      if (persisted?.phase === 'ready' && await this.dependencies.serviceReady(persisted.installRoot)) {
        ...
        this.status = {
          ...this.status,
          ...(persisted?.phase === 'ready' ? { phase: 'idle' as const, running: false, message: '完整环境已安装，但服务当前未运行；可点击一键准备安全恢复。' } : {}),
  ...
  private async inspectPrerequisites(): Promise<SetupStatus['prerequisites']> {
    const available = async (command: string, args: string[]): Promise<boolean> => {
      try { return await this.dependencies.commandAvailable(command, args) } catch { return false }
    }
  ...
  start(installRoot?: string, ragApiKey?: string): SetupStatus {
    ...
    if (!ragApiKey?.trim()) throw new Error('完整多模态需要模型 API Key；请先在当前页面填写，Key 只保存到 DSH 凭据库')
  ...
  private initialStatus(): SetupStatus {
  ...
  private async run(root: string, ragApiKey?: string): Promise<void> {
    try {
      ...
      if (missing.length) throw new Error(`缺少系统前提：${missing.join('、')}。本地开箱即用模式仍可正常使用。`)
      ...
      if (!(await exists(join(repository, 'ai-python', 'run.py')))) {
        if (await exists(repository)) throw new Error('目标项目目录已存在但不是有效项目；请选择空目录')
      ...
      if (!(await dockerContainerExists()) && await portListening(5433)) throw new Error('端口 5433 已被其他数据库或程序占用；未自动关闭或替换该服务')
      ...
      if (!(await dockerContainerExists())) {
        ...
        if (!(await dockerContainerOwned())) throw new Error(`容器 ${CONTAINER_NAME} 已存在但不属于本插件，已停止以避免误操作`)
      ...
      if (await serviceHealthy()) throw new Error('端口 8090 已被其他服务占用；未自动关闭或替换该进程')
  ...
  private statusPath(root = this.status.installRoot): string { return join(root, 'setup-status.json') }
  private logPath(root: string): string { return join(root, 'setup.log') }
  private serviceLogPath(root: string): string { return join(root, 'service.log') }

  private async update(phase: SetupPhase, message: string, running = true, error?: string): Promise<void> {
  ...
  private async readPersistedStatus(): Promise<SetupStatus | undefined> {
    for (const path of [join(this.home, 'project-knowledge-review', 'setup-pointer.json'), this.statusPath(this.defaultInstallRoot())]) {
      try {
        const value = JSON.parse(await readFile(path, 'utf8')) as SetupStatus | { installRoot?: string }
        if (!('phase' in value) && !value.installRoot) continue
        const target = 'phase' in value ? value : JSON.parse(await readFile(this.statusPath(value.installRoot!), 'utf8')) as SetupStatus
        if (target.installRoot) return target
      } catch { /* 尚未运行过一键准备。 */ }
    }
    return undefined
  }

  private async loadOrCreateSecrets(root: string): Promise<SetupSecrets> {
    const path = join(root, 'setup-secrets.json')
    try { return JSON.parse(await readFile(path, 'utf8')) as SetupSecrets } catch {
...
function validateInstallRoot(value: string): string {
  const root = resolve(value.trim())
  if (!value.trim() || !isAbsolute(root)) throw new Error('安装目录必须是本机绝对路径')
  return root
}
...
async function portListening(port: number): Promise<boolean> {
  return new Promise((resolvePromise) => {
    ...
    const finish = (value: boolean): void => { socket.destroy(); resolvePromise(value) }
...
async function commandAvailable(command: string, args: string[]): Promise<boolean> { try { await runCommand(command, args, process.cwd(), undefined); return true } catch { return false } }

async function spawnDetached(command: string, args: string[], cwd: string, env: NodeJS.ProcessEnv, logPath: string): Promise<void> {
...
async function runCommand(command: string, args: string[], cwd: string, logPath?: string, tolerateFailure = false): Promise<void> {
  await new Promise<void>((resolvePromise, rejectPromise) => {
    const child = spawn(command, args, { cwd, windowsHide: true, shell: false, env: process.env })
    let output = ''
    child.stdout?.on('data', (chunk) => { output = bounded(output + String(chunk)); if (logPath) void appendFile(logPath, chunk) })
    child.stderr?.on('data', (chunk) => { output = bounded(output + String(chunk)); if (logPath) void appendFile(logPath, chunk) })
    child.once('error', rejectPromise)
...
async function waitForCommand(command: string, args: string[], cwd: string, logPath: string, attempts: number): Promise<void> {
  for (let index = 0; index < attempts; index += 1) {
    try { await runCommand(command, args, cwd, logPath); return } catch { await delay(1000) }
  }
  throw new Error('数据库启动超时')
}

async function dockerContainerExists(): Promise<boolean> { try { await runCommand('docker', ['inspect', CONTAINER_NAME], process.cwd()); return true } catch { return false } }
async function dockerContainerOwned(): Promise<boolean> { try { return (await captureCommand('docker', ['inspect', '--format', '{{ index .Config.Labels "dsh.project-knowledge-review.managed" }}', CONTAINER_NAME])).trim() === 'true' } catch { return false } }
async function captureCommand(command: string, args: string[]): Promise<string> {
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    child.once('error', rejectPromise); child.once('close', (code) => code === 0 ? resolvePromise(output) : rejectPromise(new Error(output)))
  })
}
async function serviceHealthy(): Promise<boolean> { try { const response = await fetch(`${SERVICE_URL}/health`, { signal: AbortSignal.timeout(1500) }); return response.ok } catch { return false } }
async function ragOverviewHealthy(): Promise<boolean> { try { const response = await fetch(`${SERVICE_URL}/api/dsh-plugin/rag/overview`, { signal: AbortSignal.timeout(2000) }); return response.ok } catch { return false } }
async function ownedServiceReady(root: string): Promise<boolean> {
  try {
    const config = JSON.parse(await readFile(join(root, 'setup-config.json'), 'utf8')) as { managedBy?: string; serviceUrl?: string }
...
async function waitForHealth(seconds: number): Promise<void> { for (let index = 0; index < seconds; index += 1) { if (await serviceHealthy()) return; await delay(1000) } throw new Error('完整多模态服务启动超时，请查看 service.log') }
async function waitForRagOverview(seconds: number): Promise<void> { for (let index = 0; index < seconds; index += 1) { if (await ragOverviewHealthy()) return; await delay(1000) } throw new Error('服务健康，但 DSH 知识分区未就绪，请查看 service.log') }
async function writePrivateJson(path: string, value: unknown): Promise<void> { await writeFile(path, `${JSON.stringify(value, null, 2)}\n`, { encoding: 'utf8', mode: 0o600 }); try { await chmod(path, 0o600) } catch { /* Windows ACL 由用户目录继承。 */ } }
function bounded(value: string): string { return value.length > 12000 ? value.slice(-12000) : value }
