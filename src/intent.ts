...
export function classifyKnowledgeIntent(text: string): KnowledgeIntent {
  ...
  if (INVENTORY_PATTERNS.some((pattern) => pattern.test(normalized))) return 'knowledge-inventory'
  if (IMPORT_PATTERNS.some((pattern) => pattern.test(normalized))) return 'knowledge-import'
