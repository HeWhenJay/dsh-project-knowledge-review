...
从 `0.3.0` 起，插件安装后会在 DSH Web 的“设置”中增加“知识复习”栏目。用户可直接启停服务、切换本地/项目 RAG 模式，并配置 OCR、ASR、模型名、服务 URL 与 API Key。设置热更新，无需重新安装插件；API Key 使用 DSH 凭据库保存，页面不会回显明文。`0.3.1` 修复了 Host RAG 客户端与 Web 设置 bundle 的同名构建覆盖问题。
